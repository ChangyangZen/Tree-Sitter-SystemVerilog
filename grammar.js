...
const rules = {
  ...
  text_macro_usage: $ => prec.left(seq(
    '`',
    $.text_macro_identifier,
  ...
  data_type: $ => choice(
    seq($.integer_vector_type, optional($._signing), repeat(
	    choice(
  ...
  param_expression: $ => choice(
    $.mintypmax_expression,
    $.data_type,
    $.data_type_with_dimension,
    '$'
  ),

  data_type_with_dimension: $ => seq(
      $.data_type,                  // e.g., 'logic'
      '[', $.dimension_expression, ']'  // Handles dimensions like `[X:0]`
    ),

  dimension_expression: $ => seq(
      $.expression, ':', $.expression  // Handles `[X:0]` style expressions
    ),
